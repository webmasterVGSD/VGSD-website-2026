// Eigen invoerveld voor de beheeromgeving: een foto uploaden en met slepen bijsnijden.
//
// Wat het doet:
//  - Uploaden (of naar het veld slepen): de foto wordt eerst in de browser verkleind tot
//    maximaal 1600 pixels en als JPEG bewaard, zodat niemand een foto van 10 MB hoeft te
//    verkleinen. Bestaande foto's kunnen ook gekozen worden.
//  - Bijsnijden: je sleept de foto in een kader dat laat zien hoe het op een kaartje staat.
//    Dat bepaalt welk deel zichtbaar blijft. De foto zelf blijft heel; alleen de uitlijning
//    wordt opgeslagen.
//
// Opgeslagen waarde in data/besturen.json:  "foto": { "src": "/images/...jpg", "positie": "50% 30%" }
// ("positie" is een CSS object-position: x% y%). De website (besturen.html) leest dat.
//
// Registratie: CMS.registerFieldType('vgsd-foto', ...), gebruikt in admin/config.yml.
(function () {
  'use strict';

  if (!window.CMS || !window.createClass || !window.h) {
    console.error('vgsd-foto: Sveltia CMS is niet geladen, het foto-veld is niet beschikbaar.');
    return;
  }
  ...
  var STANDAARD = { x: 50, y: 30 };
  // Verhoudingen (breedte : hoogte) van de plekken waar de foto op de website getoond wordt.
  var KADERS = {
    kaart: 2.03,  // kaartje in het overzicht (foto 12rem hoog)
    groot: 1.17,  // groot uitgelicht blok (huidig bestuur), op een brede pagina
    popup: 2.1,   // foto boven in de popup
  };
  ...
  function leesPositie(tekst) {
    ...
    var oud = /center\s+(-?\d+(?:\.\d+)?)%/.exec(tekst || ''); // oude notatie "center 30%"
    if (oud) return { x: 50, y: begrens(parseFloat(oud[1])) };
  ...
  var Control = window.createClass({
    ...
    // --- weergave -----------------------------------------------------------------------
    kader: function (naam, positie, opties) {
      var w = this.huidig();
      opties = opties || {};
      var stijl = {
        position: 'relative',
        overflow: 'hidden',
        aspectRatio: String(KADERS[naam]) + ' / 1',
        borderRadius: '8px',
        background: 'rgba(127,127,127,0.2)',
        width: opties.breedte || '100%',
      };
      var foto = h('img', {
        src: w.src,
        alt: '',
      ...
      if (this.state.bezig) {
        ...
        inhoud = h('div', { style: { display: 'flex', flexDirection: 'column', gap: '12px' } },
          this.state.laadFout
            ...
            : h('div', null,
                this.kader('kaart', positie, { sleepbaar: true }),
  ...
  // Voorbeeldpaneel rechts in het beheerscherm: de foto zoals hij op een kaartje komt te staan.
  var Voorbeeld = window.createClass({
    render: function () {
      var w = leesWaarde(this.props.value);
      if (!w.src) return null;
      return h('div', {
        style: { aspectRatio: String(KADERS.kaart) + ' / 1', overflow: 'hidden', borderRadius: '8px', maxWidth: '420px' },
      }, h('img', {
        src: w.src,
        alt: '',
        style: {
  ...
  window.CMS.registerFieldType('vgsd-foto', Control, Voorbeeld);
})();
